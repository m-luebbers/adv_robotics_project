...
drive_msg.drive.acceleration = 5
...
def callback(data):
    ...
    x_right = x[0:int(round(lx/3))]
    x_mid = x[int(round(lx/3)):int(round(lx*2/3))]
    x_left = x[int(round(lx*2/3)):lx]

    x_right = [min(x,max_range) for x in x_right]
    x_mid = [min(x,max_range) for x in x_mid]
    x_left = [min(x,max_range) for x in x_left]

    xr_avg = np.average(x_right)
    xm_avg = np.average(x_mid)
    xl_avg = np.average(x_left)
    # print("X" ,x)
    # print("X_left",x_left)
    print("Average value of x_right", xr_avg)
    print("Average value of x_left", xl_avg)
    print("Average value of x_mid", xm_avg)
    if car_state == 0:
        if np.average(x_right) < 1:
            # drive_msg.drive.speed = 0
            s_angle = s_angle + 0.2
            drive_msg.drive.steering_angle = s_angle
            print("Turning Left")
        elif np.average(x_right) > 5:
            s_angle = s_angle - 0.2
            drive_msg.drive.steering_angle = s_angle
            print("Turning Right")
        else:
        ...
        if np.average(x_mid) < 1 and np.average(x_left) < 1 and np.average(x_right) < 1:
            car_state = 2
        if np.average(x_mid) > 7 and np.average(x_left) < 4 and np.average(x_right) < 4:
            car_state = 0
